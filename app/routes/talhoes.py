...
# ── Importar arquivo no GIS do cliente (funcionário) ────────────

@talhoes_bp.route("/funcionario/importar/<int:uid>", methods=["GET","POST"])
@employee_login_required
def gis_importar_cliente(uid):
    """Funcionário importa KML/GeoJSON para os talhões de um cliente."""
    ...
    if request.method == "POST":
        ...
        db.session.commit()
        n = len(features)
        msg = (f'Fazenda "{nome_fazenda}" importada! ({area_total:.2f} ha)'
               if n == 1 else
               f'Fazenda "{nome_fazenda}" importada com {n} talhões! '
